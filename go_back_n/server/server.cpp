...
double WAIT_TIME = 0.5 * CLOCKS_PER_SEC;
...
int recv_data(SOCKET& sockServ, SOCKADDR_IN& ClientAddr, int& ClientAddrLen, char* buf)
{
    ...
    while (1)
    {
        ...
        if ((length = recvfrom(sockServ, Buffer, sizeof(header) + MAXSIZE, 0, (sockaddr*)&ClientAddr, &ClientAddrLen))>0) 
        {
            ...
            {
                cout << "header.seq" << header.seq << " seq" << seq << endl;
                //判断接收到的包是否为预期的包
                if (header.seq != seq)
                {
                    cout << "错误的包" << endl;
                    ...
                    cout << "Not Expected PAG  !  Send ACK:" << " SEQ:" << (int)header.seq << endl;
                    continue;
                }
                //接收到正确的包
                else 
            ...
            if (clock() - start > WAIT_TIME &&seq!=begin_seq) {
